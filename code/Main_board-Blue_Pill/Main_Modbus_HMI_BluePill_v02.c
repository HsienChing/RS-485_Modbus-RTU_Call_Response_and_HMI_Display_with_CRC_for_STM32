...
/*
  Name: Main_Modbus_HMI_BluePill_v02.c
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if( HAL_UART_Receive(&huart1, Data_Modbus, sizeof(Data_Modbus), 2000) == HAL_OK ) { // When receiving data from UART1 successfully
      ...
      strcpy((char*)buf, "Data receiving SUCCESS.\r\n");
      HAL_UART_Transmit(&huart2, buf, strlen((char*)buf), 100);
      
      //Show the received Modbus data on the serial monitor through UART2
      strcpy((char*)buf, "Received Modbus data:\r\n");
      HAL_UART_Transmit(&huart2, buf, strlen((char*)buf), 100);
      ...
      // Send message to serial monitor through UART2
      strcpy((char*)buf, "Data receiving FAILED. Resend Modbus command.\r\n");
      HAL_UART_Transmit(&huart2, buf, strlen((char*)buf), 100);
    }
